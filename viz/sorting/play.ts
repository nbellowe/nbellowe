...
timerGraph(x => Math.sin(x), "red", 1);
timerGraph(x => .5 * Math.sin(.75*x), "blue", 2);
...
function timerGraph(fn, color, speed){
  ...
  var CIRCLE = false,
      PIXEL_HEIGHT = 4, PIXEL_WIDTH = 4, PIXEL_COLOR = color || "red",
      speed = speed || 1,
      STEP_BY = Math.ceil(8 * speed), STEP_EVERY = Math.ceil(50/speed),
  ...
  function drawPoint(){
      ...
      var relativeX = x/ctx.canvas.width,
          relativeY = fn(relativeX * XSCALE),
          y = Math.ceil(relativeY*YSCALE + halfHeight);
