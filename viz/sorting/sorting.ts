declare var highlightLine: (ln) => void;
declare var showActionsPanel: () => void;
declare var hideActionsPanel: () => void;
declare var showStatusPanel: () => void;
declare var hideStatusPanel: () => void;
declare var showCodetracePanel: () => void;
declare var hideCodetracePanel: () => void;
declare var triggerRightPanels: () => void;
declare var getQueryVariable: (s) => any;
declare var $: (s) => any
...
class Entry {
    ...
    highlight: string;
    position: number;
    secondaryPositionStatus: number;
    constructor(value, highlight, position, secondaryPositionStatus) {
...
class Backlink {
    ...
    secondaryPositionStatus: number;
    constructor(value, highlight, entryPosition, secondaryPositionStatus) {
        this.value = value; // number
        this.highlight = highlight; // string, use HIGHLIGHT_ constants
...
class State {
    entries: Entry[];
    backlinks: Backlink[];
    barsCountOffset: number;
    status: any;
    lineNo: any;
    constructor(entries, backlinks, barsCountOffset, status, lineNo) {
...
//Helpers
var EntryBacklinkHelper: any = new Object();
var StateHelper: any = new Object();
var FunctionList: any = new Object();
...
var makePaler = function(hexColor) {
    var red: any = Math.floor(parseInt(hexColor.slice(1, 3), 16) + 150);
    var green: any = Math.floor(parseInt(hexColor.slice(3, 5), 16) + 150);
    var blue: any = Math.floor(parseInt(hexColor.slice(5, 7), 16) + 150);
...
class Sorting {
    ...
    drawBars(state) {
        ...
        this.scaler.domain([0, d3.max(state.entries, (d: any) => d.value)]);
        ...
        canvasData
            .select("rect")
            .transition()
            .attr("height", (d: any) => this.scaler(d.value))
            .attr("width", barWidth - gapBetweenBars)
            .style("fill", (d: any) => d.highlight);
    ...
    drawCountingSortCounters(state) {
        var canvasData;
        if (state == null)
        ...
        var newData = canvasData
            ...
            .append("text")
            .attr("dy", ".35em")
            .attr("x", (d, i) => (i + 5) * barWidth + (barWidth - gapBetweenBars) / 2)
            .attr("y", 20)
            .text(d => d);
    ...
    drawRadixSortCanvas = function(state, secondaryState) {
    ...
    generateRandomNumberArray = function(size, limit) {
    ...
    generateRandomNumber = function(min, max) { //generates a random integer between min and max (both inclusive)
    ...
    createList(type) {
        ...
        this.secondaryStatelist = [null]; // the initial secondary state will be an empty state
        this.drawState(0);
    }

    setSelectedSortFunction(f) {
    ...
    radixSort() {
        ...
        this.secondaryStatelist = [false]; // showBucket flag - if true, shows the DIV's representing the bucketss
        var currentPlacing = 1;
        var targetPlacing = 1;
        var backlinkBuckets = [[], [], [], [], [], [], [], [], [], []];

        var maxValue = d3.max(state.backlinks, (d: any) => d.value);
    ...
    countingSort() {
        ...
        StateHelper.updateCopyPush(this.statelist, state);
        this.secondaryStatelist.push(secondaryState.slice()); //copy the array and push it into the secondary statelist
        ...
        StateHelper.updateCopyPush(this.statelist, state);
        this.secondaryStatelist.push(null); //copy the array and push it into the secondary statelist
    ...
    quickSortSplit(state, startIndex, endIndex) { //startIndex & endIndex inclusive
    ...
    quickSortPartition(state, startIndex, endIndex) {
    ...
    mergeSortSplitMerge(state, startIndex, endIndex) { //startIndex inclusive, endIndex exclusive
    ...
    clearPseudocode() { this.populatePseudocode([]); }

    populatePseudocode(code) {
        var i = 1;
    ...
    getAnimationDuration() { return transitionTime; }
    ...
    getCurrentIteration() { return currentStep; }

    getTotalIteration() { return this.statelist.length; }
    ...
    stop() {
        isPlaying = false;
        this.statelist = [this.statelist[0]]; //clear statelist to original state, instead of new Array();
