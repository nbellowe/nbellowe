...
makeComboInput("type", function (v) {
    clearTimers();
    alg = getAlg(v);
    draw();
});
...
function quicksort(items, left, right, i, j) {
    var index;
    i = i || 0;
    i = j || 0;
...
function mergeSort() {
    ...
    if (length < 2)
        return this;
    return merge(this.slice(0, middle).mergeSort(compareFn), this.slice(middle, length).mergeSort(compareFn));
}
