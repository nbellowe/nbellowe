...
// Devare folders and files recursively
file.devare = function(filepath, options) {
  ...
  grunt.verbose.write((nowrite ? 'Not actually devaring ' : 'Devaring ') + filepath + '...');

  if (!file.exists(filepath)) {
    grunt.verbose.error();
    grunt.log.warn('Cannot devare nonexistent file.');
    return false;
  }

  // Only devare cwd or outside cwd if --force enabled. Be careful, people!
  if (!options.force) {
    if (file.isPathCwd(filepath)) {
      grunt.verbose.error();
      grunt.fail.warn('Cannot devare the current working directory.');
      return false;
    } else if (!file.isPathInCwd(filepath)) {
      grunt.verbose.error();
      grunt.fail.warn('Cannot devare files outside the current working directory.');
      return false;
    }
  }

  try {
    // Actually devare. Or not.
    if (!nowrite) {
    ...
    grunt.verbose.error();
    throw grunt.util.error('Unable to devare "' + filepath + '" file (' + e.message + ').', e);
  }
};
